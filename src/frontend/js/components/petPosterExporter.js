...
export class PetPosterExporter extends HTMLElement {
    ...
    async setPosterQr(pet) {
        ...
        const detailUrl = this.getPetDetailUrl(pet.id);
        qrImg.src = await this.generateQrDataUrl(detailUrl);
        qrImg.alt = `QR del anuncio de ${pet.nombre || 'la mascota'}`;
    }

    getPetDetailUrl(petId) {
    ...
    async generateQrDataUrl(url) {
        if (!window.QRCode) {
            throw new Error('La libreria QRCode no esta cargada.');
        }
    ...
    getPosterEventDate(pet) {
        ...
        return eventDate ? new Date(eventDate).toLocaleDateString('es-ES') : 'Fecha no disponible';
    }
